...
modified: Saturday, July 8th 2023, 4:08:42 pm  
tags:  
  - shapecreations  
media: ""  
status: in-progress  
...
There are two types of volumes, each with their own use cases. The main thing to remember is that with a volume, there is a link between some folder on the host computer and a folder within the container. When declaring a volume, the connection is made within the image build step but does not become active until container runtime.  
